...
public:
    ...
    bool add_point(const X &x, const Y &y) {
        ...
        //add the second point, which can settle the slope
        if (points_in_hull == 1) {
            rectangle[2] = p2;
            rectangle[3] = p1;
            upper.push_back(p1);
...
size_t make_segmentation(size_t n, size_t epsilon, Fin in, Fout out, std::vector<uint64_t> & array_sizes) {
    if (n == 0)
        return 0;
...
template<typename Fin, typename Fout>
size_t make_segmentation_par(size_t n, size_t epsilon, Fin in, Fout out, std::vector<uint64_t>& array_sizes ) {
    ...
    if (parallelism == 1 || n < 1ull << 15)
        return make_segmentation(n, epsilon, in, out, array_sizes);
    //@ this part not consider yet
    c = -1;
    assert(c >= 0);
    return c;
    // using X = typename std::invoke_result_t<Fin, size_t>::first_type;
